...
class pair_fourier_test (unittest.TestCase):
    ...
    def python_fourier_calculation(self, dx, dy, dz, fourier_a, fourier_b, degree=9, r_cut=3.0):
        ...
        if r > r_cut:
           ...
           for i in range(0, degree-1):
               a1 += fourier_a[i] * (-1)**i
               # b1 += fourier_b[i] * (i+n_shift) * (-1)**i
           energy += a1 * np.cos(np.pi*r/r_cut)
           # energy += b1 * np.sin(np.pi*r/r_cut)
           force += a1 * np.sin(np.pi*r/r_cut)*np.pi/r_cut
           # force -= b1 * np.cos(np.pi*r/r_cut)*np.pi/r_cut
           for i in range(0, degree-1):
    ...
    def test_pair_fourier_value(self):
        ...
        # print(force_x, force_y, force_z, energy)
        ...
        # print(force_x, force_y, force_z, energy)
        # print(force_fourier_2)
        # print(potential_fourier_2)
