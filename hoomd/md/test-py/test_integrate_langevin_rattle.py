...
class integrate_langevin_rattle_tests (unittest.TestCase):
    def setUp(self):
        print
    ...
    def test_basic_run(self):
        ...
        run(5);
        bd.disable();
        bd = md.integrate.langevin_rattle(all, manifold=sphere, kT=1.2, seed=1, dscale=1.0);
        run(5);
        bd.disable();
        bd = md.integrate.langevin_rattle(all, manifold=sphere, kT=1.2, seed=1, dscale=1.0, tally=True);
        run(5);
        bd.disable();
        bd = md.integrate.langevin_rattle(all, manifold=sphere, kT=1.2, seed=1, dscale=1.0, noiseless_t=True);
        run(5);
        bd.disable();
        bd = md.integrate.langevin_rattle(all, manifold=sphere, kT=1.2, seed=1, dscale=1.0, noiseless_r=True);
        run(5);
        bd.disable();
        bd = md.integrate.langevin_rattle(all, manifold=sphere, kT=1.2, seed=1, dscale=1.0, noiseless_t=True, noiseless_r=True);
        run(5);
        bd.disable();
...
class integrate_langevin_rattle_gyroid (unittest.TestCase):
    def setUp(self):
        print
...
class integrate_langevin_rattle_diffusion (unittest.TestCase):
    def setUp(self):
        print
    ...
    def test_noiseless_t(self):
        ...
        if comm.get_rank() == 0:
            ...
            vsq = np.mean(snap.particles.velocity[:,0] * snap.particles.velocity[:,0] +
    ...
    def test_gamma(self):
        ...
        if comm.get_rank() == 0:
            ...
            self.assert_(almost_equal(vsq, 2*kT, 0.1))
